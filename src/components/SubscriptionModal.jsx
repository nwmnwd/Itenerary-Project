...
// Sub-component for Payment Process
const PaymentForm = ({ onPaymentSuccess, onCancel }) => {
  ...
  const handlePay = () => {
    ...
    // Simulate 2-second payment process
    setTimeout(() => {
      setLoading(false);
      setIsSuccess(true); // Set status to success

      // Wait 1.5 seconds for the user to see the checkmark, then close the modal
      setTimeout(() => {
        onPaymentSuccess(); // Call success function
      }, 1500);
    }, 2000);
  };
    ...
    <div className="h-full rounded-xl bg-white p-6 text-center shadow-xl dark:bg-gray-800">
      {isSuccess ? (
        <div className="flex flex-col items-center justify-center px-20 py-12">
          {/* Checkmark Icon (SVG) - Adding animate-bounce for a cool effect */}
          <svg
...
export default function SubscriptionModal({ onPaymentSuccess, onClose }) {
  const [stage, setStage] = useState("plan"); // 'plan' | 'payment'
  ...
  const handleCancel = () => {
    // Close the modal if in the plan stage, go back to plan if in the payment stage
    if (stage === "payment") {
      setStage("plan");
    } else {
      onClose();
        ...
        {stage === "plan" && (
          <div className="relative isolate rounded-xl bg-white px-6 py-8 shadow-2xl sm:py-16 lg:px-8 dark:bg-gray-900">
            ...
            <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20">
              ...
              {/* Premium Plan (Adapted from Enterprise) */}
